...
### new one
data_all_lapses <- readRDS(file=paste0(outputDataPath,bootstrap_CI,"prod_r1000_rpnal_abvol_abvol_corrado_df.rds"))
print("profiles database dimension")
print(dim(data_all_lapses))
print("number of lapsees computed")
print(unique(data_all_lapses$lapse))

data_all_lapses$localSource[data_all_lapses$localSource == "DJPR"] <- "DJPR"
data_all_lapses$localSource[data_all_lapses$localSource == "PREMIUM"] <- "PREMIUM_PACK"
data_all_lapses$localSource[data_all_lapses$localSource == "WEBNONPREMIUM"] <- "WEB_NON_PREMIUM"


aggregated_data <- NULL
for (my_lapse in c(-1,2,3,4,5)){
  data <- data_all_lapses[data_all_lapses$lapse == my_lapse,]
  
  print("Lapse")
  print(unique(data$lapse))

  print("Lapse dimension")
  print(dim(data))
  ...
  print("Spread added data dimension")
  print(dim(data))
  ...
  for(my_metric in my_metrics){
    ...
    gpfilename <- paste0(my_lapse,my_metric,"rpnaf_best_group")
    ...
    catfilename <- paste0(my_lapse,my_metric,"rpnaf_best_category")
...
RP_SaveDataFrame(aggregated_data, outputDataPath = outputDataPath, filename = "all_lapses_rpna_metrics_clean_prod")