...
# data <- readRDS(file=paste0(outputDataPath,"FALSElprod_r1000_bigdataff_abvol_abvol_corrado_df.rds"))
print(dim(data))
...
my_lapses_files <- 
  list(
    ...
    "3" = "FALSEl4prod_r1000_bigdata_abvol_abvol_corrado_df.rds",
...
for (my_lapse in c(-1,2,3,4,5)){
  ...
  for(my_metric in my_metrics){
    ...
    gpfilename <- paste0(my_lapse,my_metric,"bigdataf_best_group")
    ...
    catfilename <- paste0(my_lapse,my_metric,"bigdataf_best_category")
...
RP_SaveDataFrame(aggregated_data, outputDataPath = outputDataPath, filename = "all_lapses_bigdata_metrics_clean_prod")