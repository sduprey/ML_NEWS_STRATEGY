...
bootstrap_CI <- FALSE
data <- readRDS(file=paste0(outputDataPath,bootstrap_CI,"new_r1000_bigdata_abvol_abvol_corrado_df.rds"))
print(dim(data))
...
RP_SaveDataFrame(data_spread_augmented, outputDataPath = outputDataPath, filename = "new_spr_r1000_bigdata_abvol_abvol_corrado_df")
print("spread saved")
...
data <- readRDS(file=paste0(outputDataPath,"new_spr_r1000_bigdata_abvol_abvol_corrado_df.rds"))
...
RP_SaveDataFrame(all_group_events, outputDataPath = outputDataPath, filename = "new_bigdata_all_group_events")
RP_SaveDataFrame(all_category_events, outputDataPath = outputDataPath, filename = "new_bigdata_all_category_events")
...
RP_SaveDataFrame(dataGroupDT, outputDataPath = outputDataPath, filename = "new_bigdata_best_profile_group_ordered_r1000_corrado_df")
...
RP_SaveDataFrame(dataCategoryDT, outputDataPath = outputDataPath, filename = "new_bigdata_best_profile_category_ordered_r1000_corrado_df")
...
for (cor in unique(big_data$corrado_methodo)){
  for(lap in unique(big_data$lapse)){
    for(rel in unique(big_data$relevance)){
      for(evrel in unique(big_data$event_relevance)){
        for(evt in unique(big_data$my_event)){
          for(sent in unique(big_data$sentiment_criteria)){
            for(sour in unique(big_data$localSource)){
              for(sim in unique(big_data$similarity_gap_filter)){
                bid_index <- 
