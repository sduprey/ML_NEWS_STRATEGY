...
# fill ihn those who don t have a joining date
...
##### gross income =====> missing a lot of values
sum(is.na(df$renta))


############ assigning missing incomes by province
new.incomes <-df %>%
  select(nomprov) %>%
  merge(df %>%
          group_by(nomprov) %>%
...
############## For example, it seems every entry from Alava has NA for renta.
df$renta[is.na(df$renta)] <- median(df$renta,na.rm=TRUE)
df <- arrange(df,fecha_dato)


## still remaining value for
sum(is.na(df$ind_nomina_ult1))
### cheap way : nullify every thing
df[is.na(df)] <- 0
...
########## convert all the features to dummy variables ////// check their distribution
...
# I will create a label for each product and month that indicates whether a customer added,
# dropped or maintained that service in that billing cycle. 
# I will do this by assigning a numeric id to each unique time stamp, 
# and then matching each entry with the one from the previous month. 
# The difference in the indicator value for each product then gives the desired value.
# A cool trick to turn dates into unique id numbers is to use as.numeric(factor(...)).
# Make sure to order them chronologically first.
...
######## applying to it to all features
df[,features] <- lapply(df[,features], function(x) return(ave(x,df$ncodpers, FUN=status.change)))

########### trim away the maintained to get the gist : changing behaviour is what matters
...
######################### visualization of the ratio per month
