...
getPlotBestProfilesTogether <- function(aggregate_criteria, data, all_group_events, all_category_events,suffix, xlim = FALSE){
  plotLimit <- 6
  if (aggregate_criteria == "GROUP"){
    plotLimit <- 6
    all_events <- all_group_events
...
data <- data[!grepl("relative-strength-index-overbought",data$my_event),]
...
for(my_metric in my_metrics){
  ...
  gname <- ExportMultiplot(gretgp1,gvolgp1,
                           gretcat1,gvolcat1,
                           plotlist = NULL, filename = paste0("bests",my_metric), outputDataPath = paste0(outputDataPath,"PAPER_PICTURES/"), cols = 2, width = width, height = height)
