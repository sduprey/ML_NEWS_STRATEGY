                           ...
                           FullExportingPath = NULL) 
{
  ...
  if (pointOnly) {
    ...
    g <- g + geom_line(size = 1.5) + geom_point()
  }
...
outputGraphicsBestProfileRets <- function(product_criteria,aggregate_criteria,sentiment_criteria,similarity_gap_filter,ens_filter,event_number_event_filtering, gics_sector, my_event, localSource, dataFrame,  plotInArborescence, Russell_version = "R1000", Together = FALSE){
  ...
  dataframerets <- dataframerets[abs(dataframerets$MINUTES) <= 120,]
...
outputGraphicsBestProfileVol <- function(product_criteria,aggregate_criteria,sentiment_criteria,similarity_gap_filter,ens_filter,event_number_event_filtering, gics_sector, my_event, localSource, dataFrame,  plotInArborescence, Russell_version = "R1000", Together = FALSE){
  ...
  dataframevol <- dataframevol[abs(dataframevol$MINUTES) <= 120,]
  # g1 <- RP_PlotDataFrame(dataframestats,AxisIncluded = T,XLab = "Minute Lags",YLab = "p-value",Title = my_event, FullExportingPath = NULL)
...
getPlotBestProfilesOneByOne <- function(aggregate_criteria, dataT, all_group_events, all_category_events,suffix){
  ...
  if ( dim(dataT)[1] > 0){
    ...
    for (i in 1:min(plotLimit, dim(dataT)[1])){
      ...
      gret <- outputGraphicsBestProfileRets(rowProfile$product_criteria,rowProfile$aggregate_criteria,rowProfile$sentiment_criteria,rowProfile$similarity_gap_filter,rowProfile$ens_filter,rowProfile$event_number_event_filtering, rowProfile$gics_sector, rowProfile$my_event, rowProfile$localSource, dataFrame = dataframe, FALSE, Russell_version = "R1000")
      gvol <- outputGraphicsBestProfileVola(rowProfile$product_criteria,rowProfile$aggregate_criteria,rowProfile$sentiment_criteria,rowProfile$similarity_gap_filter,rowProfile$ens_filter,rowProfile$event_number_event_filtering, rowProfile$gics_sector, rowProfile$my_event, rowProfile$localSource, dataFrame = dataframe, FALSE, Russell_version = "R1000")
      my_source <- rowProfile$localSource
      my_event <- rowProfile$my_event
      RP_ExportPlot(gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/",suffix,"/"), filename = paste0(my_source,my_event,suffix,i,"best_return"))
      RP_ExportPlot(gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/",suffix,"/"), filename = paste0(my_source,my_event,suffix,i,"best_volatility"))
...
colnames(MappingGroupCategory_RPData) <- c("GROUP","my_event")

all_data <- readRDS(file=paste0(outputDataPath,"metrics_clean_prod_spr_r1000_bigdataf_abvol_abvol_corrado_df.rds"))
print(dim(all_data))

all_data <- all_data[all_data$similarity_gap_filter == 1,]
all_data <- all_data[all_data$event_relevance == "HIGH",]
all_data <- all_data[all_data$relevance == "HIGH",]
all_data <- all_data[all_data$sentiment_criteria != "SPREAD",]


all_data <- all_data[!grepl("analyst-ratings-change-neutral",all_data$my_event),]
all_data <- all_data[!grepl("dividend-below-expectations",all_data$my_event),]
...
all_data <- all_data[!grepl("campaign-ad-release",all_data$my_event),]
all_data <- all_data[!grepl("donation",all_data$my_event),]
all_data <- all_data[!grepl("bankruptcy-exit",all_data$my_event),]
all_data <- all_data[!grepl("bankruptcy-unit",all_data$my_event),]
...
# groupsToInvestigate <- 
#   list(
#     "card_post_ranked_return" = c("analyst-ratings","insider-trading","credit-rating"),
#     "post_ranked_return" = c("dividends", "analyst-ratings","marketing"),
#     "volatility_correction" = c("security","equity-actions","exploration")
#   )

card_post_ranked_return_list = c("analyst-ratings","insider-trading","credit-ratings")
post_ranked_return_list = c("dividends", "corporate-responsibility","bankruptcy")
volatility_correction_list = c("security","equity-actions","exploration")
...
my_group_list <- unique(c(card_post_ranked_return_list,post_ranked_return_list,volatility_correction_list,"analyst-ratings","credit-ratings","insider-trading","marketing","corporate-responsibility","exploration","security","equity-actions"))


dataF <- all_data[all_data$aggregate_criteria == "GROUP",]


# all_data <-  merge(all_data,MappingGroupCategory_RPData, by="my_event")
...
for(my_metric in my_metrics){
  ...
  print("Filtering our events")
  # print(unique(all_data$GROUP))
  
  #   my_filter <- NULL
  #   my_group_list <- NULL 
  #   if(my_metric == "card_post_ranked_return"){
  #     my_filter <- (all_data$GROUP %in% card_post_ranked_return_list)
  #     my_group_list <- card_post_ranked_return_list
  #   }
  #   
  #   if(my_metric == "post_ranked_return"){
  #     my_filter <- (all_data$GROUP %in% post_ranked_return_list)
  #     my_group_list <- post_ranked_return_list
  #   }
  #   
  #   if(my_metric == "volatility_correction"){
  #     my_filter <- (all_data$GROUP %in% volatility_correction_list) 
  #     my_group_list <- volatility_correction_list
  #   } 
  
  #   print("Remaining")
  #   print(sum(my_filter))
  #   
  #   dataF <- all_data[my_filter,]
  
  # print(dim(dataF))
  # print(unique(dataF$GROUP))
  
  if(my_metric == "card_post_ranked_return"){
    card_significance <- dataF$event_number_event_filtering
    card_significance <- (card_significance-min(card_significance))/(max(card_significance)-min(card_significance))
    
    dataF$RANKING <- card_significance*dataF$post_ranked_return
    dataF$card_post_ranked_return <-   dataF$RANKING
  } else {
    dataF$RANKING <- dataF[,my_metric]
  }
  
  print("before")
  print(dim(dataF))
  
  dataMetricTrimmed  <- ddply(.data = dataF,.variables = "my_event",.fun = function(x){trimBest(x)})
  
  print("after")
  print(dim(dataMetricTrimmed))
  
  dataMetricTrimmed <- dataMetricTrimmed[order(dataMetricTrimmed$RANKING,decreasing = TRUE),]
  
  for (my_group in my_group_list){
    print("Dealing with group : ")
    print(my_group)
    dataMetricTrimmedC <- dataMetricTrimmed[dataMetricTrimmed$my_event == my_group,]
    
    dataMetricTrimmedC <- dataMetricTrimmedC[order(dataMetricTrimmedC$RANKING,decreasing = TRUE),]
    # 
    #     results <- getPlotBestProfilesOneByOne("GROUP",dataMetricTrimmed,all_group_events,all_category_events,my_metric)
    #     print(results$gret)
    ...
    results <- getPlotBestProfilesOneByOne("GROUP",dataMetricTrimmedC,all_group_events,all_category_events,my_metric)
    #     print(results$gret)
