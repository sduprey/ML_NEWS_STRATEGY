...
RP_PlotCIIntervalSuperposed <- function(DataFramePos, DataFrameNeg, Title = "", FullExportingPath=NULL) {
  ...
  g <- ggplot(DataFrame)+
    geom_line(data=DataFrame, aes(x=MINUTES, y=POS_MEAN), size=1.2, colour="blue")+
    geom_ribbon(data=DataFrame,aes(x=MINUTES,ymin=POS_CI_LOW,ymax=POS_CI_HIGH),alpha=0.3)+
    geom_line(data=DataFrame, aes(x=MINUTES, y=NEG_MEAN), size=1.2, colour="red")+
    geom_ribbon(data=DataFrame,aes(x=MINUTES,ymin=NEG_CI_LOW,ymax=NEG_CI_HIGH),alpha=0.3)+
    xlab("Minute Lags") + 
    ylab("BPS cumulated minute volume") +
    ggtitle(Title)+
    theme(axis.text=element_text(size=16),
...
data <- readRDS(file=paste0(outputDataPath,"metrics_clean_prod_spr_r1000_rpna_abvol_abvol_corrado_df.rds"))
...
data <- data[data$localSource == "DJPR",]
...
for(my_metric in my_metrics){
  print(my_metric)
  # dataGroupPos <- dataGroup[dataGroup$sentiment_criteria =="POSITIVE" | dataGroup$sentiment_criteria =="SPREAD",]
  dataGroupPos <- dataGroup[dataGroup$sentiment_criteria =="POSITIVE",]
  
  
  # dataGroupPos <- dataGroup
  
  dataGroupPosBest  <- ddply(.data = dataGroupPos,.variables = "my_event",.fun = function(x){trimBest(x,my_metric)})
  print("after")
  print(dim(dataGroupPosBest))
  
  
  stats_prepost <- colnames(dataGroupPosBest)[which(as.numeric(colnames(data))>= -180)]
  
  ret_prepost <- paste0("RET",stats_prepost)
  vol_prepost <- paste0("VOLA",stats_prepost)
  volu_prepost <- paste0("VOLU",stats_prepost)  
  
  allNumericColumns <- c(stats_prepost,ret_prepost,vol_prepost,volu_prepost)
  allRetsColumns <- ret_prepost
  
  #   print("problem in the code before : rescale")
  # dataGroupPosBest[,allRetsColumns] <- dataGroupPosBest[,allRetsColumns]*10
  
  
  my_df <- dataGroupPosBest[1:clean_counter,allNumericColumns]
  my_diff_df <- differentiateRets(dataGroupPosBest[1:clean_counter,allRetsColumns])
  ...
  rowProfilePos <- dataGroupPosBest[1:3,,drop=FALSE]
  rowProfilePos[1,allNumericColumns] <- event_minutes_ci_matrix[1,]
  rowProfilePos[2,allNumericColumns] <- event_minutes_ci_matrix[2,]
  rowProfilePos[3,allNumericColumns] <- event_minutes_ci_matrix[3,]
  
  
  #   rowProfilePos[1,allRetsColumns] <-  event_minutes_ci_matrix[2,allRetsColumns] - abs(event_minutes_diff_ci_matrix[1,])
  #   # rowProfile[2,allRetsColumns] <- event_minutes_ci_matrix[2,]
  #   rowProfilePos[3,allRetsColumns] <- event_minutes_ci_matrix[2,allRetsColumns] + abs(event_minutes_diff_ci_matrix[3,])
  ...
  rowProfilePos[1,allRetsColumns] <-  event_minutes_ci_matrix[2,allRetsColumns] - lowerbound
  # rowProfile[2,allRetsColumns] <- event_minutes_ci_matrix[2,]
  rowProfilePos[3,allRetsColumns] <- event_minutes_ci_matrix[2,allRetsColumns] + upperbound
  
  ###########3
  
  
  dataGroupNeg <- dataGroup[dataGroup$sentiment_criteria =="NEGATIVE",]
  
  
  # dataGroupNeg <- dataGroup
  
  dataGroupNegBest  <- ddply(.data = dataGroupNeg,.variables = "my_event",.fun = function(x){trimBest(x,my_metric)})
  print("after")
  print(dim(dataGroupNegBest))
  
  
  stats_prepost <- colnames(dataGroupNegBest)[which(as.numeric(colnames(data))>= -180)]
  ...
  #   print("problem in the code before : rescale")
  # dataGroupNegBest[,allRetsColumns] <- dataGroupNegBest[,allRetsColumns]*10
  
  
  my_df <- dataGroupNegBest[1:clean_counter,allNumericColumns]
  my_diff_df <- differentiateRets(dataGroupNegBest[1:clean_counter,allRetsColumns])
  ...
  rowProfileNeg <- dataGroupNegBest[1:3,,drop=FALSE]
  rowProfileNeg[1,allNumericColumns] <- event_minutes_ci_matrix[1,]
  rowProfileNeg[2,allNumericColumns] <- event_minutes_ci_matrix[2,]
  rowProfileNeg[3,allNumericColumns] <- event_minutes_ci_matrix[3,]
  
  #   rowProfileNeg[1,allRetsColumns] <-  event_minutes_ci_matrix[2,allRetsColumns] - abs(event_minutes_diff_ci_matrix[1,])
  #   # rowProfile[2,allRetsColumns] <- event_minutes_ci_matrix[2,]
  #   rowProfileNeg[3,allRetsColumns] <- event_minutes_ci_matrix[2,allRetsColumns] + abs(event_minutes_diff_ci_matrix[3,])
  #   
  #################
  ...
  rowProfileNeg[1,allRetsColumns] <-  event_minutes_ci_matrix[2,allRetsColumns] - lowerbound
  # rowProfile[2,allRetsColumns] <- event_minutes_ci_matrix[2,]
  rowProfileNeg[3,allRetsColumns] <- event_minutes_ci_matrix[2,allRetsColumns] + upperbound
  ...
  resultsPos <- RP_PlotProfile(rowProfilePos) 
  print(resultsPos$gret)
  resultsNeg <- RP_PlotProfile(rowProfileNeg)
  print(resultsNeg$gret)
  
  results <- RP_PlotProfileTogether(rowProfilePos,rowProfileNeg)
  RP_ExportPlot(results$gret,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/BEST_PROFILES/",my_metric,"split_bigdata_returns_group_average_profile_ci"))
  print(results$gret)
