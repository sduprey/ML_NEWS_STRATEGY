                           ...
                           FullExportingPath = NULL, xlim = FALSE,minZoom = FALSE) 
{
  ...
  if(showLegend){
    ToPlotDataFrame$Days <- ToPlotDataFrame$variable
    g <- ggplot(ToPlotDataFrame, aes(x = ToPlotDataFrame[, c(my_column_to_plot_against)], 
                                     y = value, group = Days, color = Days, fill = Days))
    if (pointOnly) {
      g <- g + geom_point()
    }
    else if (lineOnly) {
    ...
    g <- g +theme(legend.position = c(0.1, 0.8),legend.text = element_text(size = 30),legend.title = element_text("Days"))+#    theme( legend.position = "bottom")+
      theme(legend.background = element_rect(fill = "lightgrey",linetype="solid"))+
      theme(legend.title = element_text(size = 30))+#face = "italic"))+
      theme(axis.text=element_text(size=30),
            axis.title=element_text(size=30,face="bold")) +
      theme(plot.title = element_text(size = 30, face = "bold"))+
      theme(legend.key.size = unit(1.25, "cm"))
    g <- g + scale_fill_discrete(guide = guide_legend(title = "V"))
  } else {
    g <- ggplot(ToPlotDataFrame, aes(x = ToPlotDataFrame[, c(my_column_to_plot_against)], 
  ...
  # g <- g + theme(legend.position = "bottom",legend.text = element_text(size = 30),legend.title = element_blank(),legend.direction="vertical")
  
  #     theme(legend.position = c(0.1, 0.9), legend.box = "vertical", 
...
my_source <- "ALL"
dataSource <- all_data[(all_data$localSource == "DJPR" | all_data$localSource == "PREMIUM_PACK"),]

short <- "long"
doSimilarity <- TRUE
# similarity_filter <- "FILTER"
similarity_filter <- "ALL"
if(doSimilarity){
  ...
  for(my_metric in my_metrics){
    ...
    # my_similarity_gaps <- c(0,1,90,365)
    if(similarity_filter == "ALL"){
      my_similarity_gaps <- c(0,1,7,30,90,186,365)
      
    } else {
      my_similarity_gaps <- c(0,1,7)
    }
    ...
    if(my_metric == "card_post_ranked_return"){
      ...
      agg_rowProfileCtrashh[,stats_post_sign] <- factor*agg_rowProfileCtrash[,stats_post_sign]
    ...
    print(results$gret)
    print(results$gvol)
    RP_ExportPlot(results$gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SIM_FILTER/CATEGORY/"), filename = paste0(similarity_filter,trash_counter,my_source,my_metric,"bigdata_similarity_trash_category_ep_return"))
    RP_ExportPlot(results$gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SIM_FILTER/CATEGORY/"), filename = paste0(similarity_filter,trash_counter,my_source,my_metric,"bigdata_similarity_trash_category_ep_volatility"))
    RP_ExportPlot(results$gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SIM_FILTER/CATEGORY/"), filename = paste0(similarity_filter,my_source,my_metric,"CAT_RET_SIM"))
    RP_ExportPlot(results$gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SIM_FILTER/CATEGORY/"), filename = paste0(similarity_filter,my_source,my_metric,"CAT_VOL_SIM"))
    
    #   
    results <- RP_PlotMetricsTogether(agg_rowProfileGtrash,"GROUP")
    print(results$gret)
    print(results$gvol)
    RP_ExportPlot(results$gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SIM_FILTER/GROUP/"), filename = paste0(similarity_filter,trash_counter_group,my_source,my_metric,"bigdata_similarity_trash_group_ep_return"))
    RP_ExportPlot(results$gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SIM_FILTER/GROUP/"), filename = paste0(similarity_filter,trash_counter_group,my_source,my_metric,"bigdata_similarity_trash_group_ep_volatility"))
    #   
#     results <- RP_PlotMetricsTogether(agg_rowProfileC,"CATEGORY")
#     print(results$gret)
#     print(results$gvol)
#     RP_ExportPlot(results$gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SIM_FILTER/CATEGORY/"), filename = paste0(similarity_filter,clean_counter,my_source,my_metric,"bigdata_similarity_clean_category_ep_return"))
#     RP_ExportPlot(results$gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SIM_FILTER/CATEGORY/"), filename = paste0(similarity_filter,clean_counter,my_source,my_metric,"bigdata_similarity_clean_category_ep_volatility"))
#     #   
#     results <- RP_PlotMetricsTogether(agg_rowProfileG,"GROUP")
#     print(results$gret)
#     print(results$gvol)
#     RP_ExportPlot(results$gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SIM_FILTER/GROUP/"), filename = paste0(similarity_filter,clean_counter_group,my_source,my_metric,"bigdata_similarity_clean_group_ep_return"))
#     RP_ExportPlot(results$gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SIM_FILTER/GROUP/"), filename = paste0(similarity_filter,clean_counter_group,my_source,my_metric,"bigdata_similarity_clean_group_ep_volatility"))
#     RP_ExportPlot(results$gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SIM_FILTER/GROUP/"), filename = paste0(similarity_filter,my_source,my_metric,"GROUP_RET_SIM"))
#     RP_ExportPlot(results$gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SIM_FILTER/GROUP/"), filename = paste0(similarity_filter,my_source,my_metric,"GROUP_VOL_SIM"))
...
if(doEVTConfidence){
  ...
  for(my_metric in my_metrics){
    ...
    #     print("displaying all metrics together")
    #     if(my_source == "DJPR"){
    #       print(dim(agg_rowProfileCtrash))
    #       rets <- paste0("RET",colnames(agg_rowProfileCtrash)[which(!is.na(as.numeric(colnames(agg_rowProfileCtrash))))])
    #       temp <- agg_rowProfileCtrash[3,rets]
    #       agg_rowProfileCtrash[3,rets] <- (agg_rowProfileCtrash[2,rets] +agg_rowProfileCtrash[3,rets])/2
    #       agg_rowProfileCtrash[2,rets] <- temp
    #       
    #     }
    #     my_revert <- c(2,1,3)
    #     
    #     agg_rowProfileCtrash <- agg_rowProfileCtrash[my_revert,]
    #     agg_rowProfileC <- agg_rowProfileC[my_revert,]
    #     
    #     agg_rowProfileGtrash <- agg_rowProfileGtrash[my_revert,]
    #     agg_rowProfileG <- agg_rowProfileG[my_revert,]
    ...
    if(my_metric == "post_ranked_return"){
      ...
      agg_rowProfileCtrashh[2,stats_post_sign] <- factor3*agg_rowProfileCtrash[2,stats_post_sign]
    ...
    print(results$gret)
    print(results$gvol)
    RP_ExportPlot(results$gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/REL_FILTER/CATEGORY/"), filename = paste0(trash_counter,my_source,my_metric,"bigdata_event_relevance_trash_category_return"))
    RP_ExportPlot(results$gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/REL_FILTER/CATEGORY/"), filename = paste0(trash_counter,my_source,my_metric,"bigdata_event_relevance_trash_category_volatility"))
    RP_ExportPlot(results$gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/REL_FILTER/CATEGORY/"), filename = paste0(my_source,my_metric,"CAT_RET_REL"))
    RP_ExportPlot(results$gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/REL_FILTER/CATEGORY/"), filename = paste0(my_source,my_metric,"CAT_VOL_REL"))
    
    #   
    results <- RP_PlotMetricsTogether(agg_rowProfileGtrash,"GROUP")
    print(results$gret)
    print(results$gvol)
    RP_ExportPlot(results$gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/REL_FILTER/GROUP/"), filename = paste0(trash_counter_group,my_source,my_metric,"bigdata_event_relevance_trash_group_return"))
    RP_ExportPlot(results$gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SIM_FILTER/GROUP/"), filename = paste0(trash_counter_group,my_source,my_metric,"bigdata_event_relevance_trash_group_volatility"))
    #   
    results <- RP_PlotMetricsTogether(agg_rowProfileC,"CATEGORY")
    print(results$gret)
    print(results$gvol)
    RP_ExportPlot(results$gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/REL_FILTER/CATEGORY/"), filename = paste0(clean_counter,my_source,my_metric,"bigdata_event_relevance_clean_category_return"))
    RP_ExportPlot(results$gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/REL_FILTER/CATEGORY/"), filename = paste0(clean_counter,my_source,my_metric,"bigdata_event_relevance_clean_category_volatility"))
    #   
    results <- RP_PlotMetricsTogether(agg_rowProfileG,"GROUP")
    print(results$gret)
    print(results$gvol)
    RP_ExportPlot(results$gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/REL_FILTER/GROUP/"), filename = paste0(clean_counter_group,my_source,my_metric,"bigdata_event_relevance_clean_group_return"))
    RP_ExportPlot(results$gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/REL_FILTER/GROUP/"), filename = paste0(clean_counter_group,my_source,my_metric,"bigdata_event_relevance_clean_group_volatility"))
    RP_ExportPlot(results$gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/REL_FILTER/GROUP/"), filename = paste0(my_source,my_metric,"GROUP_RET_REL"))
    RP_ExportPlot(results$gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/REL_FILTER/GROUP/"), filename = paste0(my_source,my_metric,"GROUP_VOL_REL"))
...
if(doSources){
  ...
  for(my_metric in my_metrics){
    ...
    for (my_source in c("DJPR","PREMIUM_PACK","WEB_NON_PREMIUM")){
      ...
      data <- dataSource[dataSource$event_relevance == "HIGH",]
      data <- data[data$similarity_gap_filter == 1,]
      data <- data[data$relevance == "HIGH",]
      data <- data[data$sentiment_criteria != "SPREAD",]
      ...
      ############# Actual plotting aggregation for groups
      rowProfileG <- dataMetricTrimmedG[1,]
      rowProfileG$my_event <- my_source
      stats_sign <- colnames(rowProfileG)[which(!is.na(as.numeric(colnames(rowProfileG))))]
      rets <- paste0("RET",colnames(rowProfileG)[which(!is.na(as.numeric(colnames(rowProfileG))))])
      ord_stats_sign <- paste0("ORD",colnames(rowProfileG)[which(!is.na(as.numeric(colnames(rowProfileG))))])
      vol_stats_sign <- paste0("VOLU",colnames(rowProfileG)[which(!is.na(as.numeric(colnames(rowProfileG))))])
      ...
      rowProfileGtrash <- dataMetricTrimmedG[1,]
      rowProfileGtrash$my_event <- my_source
      stats_sign <- colnames(rowProfileGtrash)[which(!is.na(as.numeric(colnames(rowProfileGtrash))))]
      rets <- paste0("RET",colnames(rowProfileGtrash)[which(!is.na(as.numeric(colnames(rowProfileGtrash))))])
      ord_stats_sign <- paste0("ORD",colnames(rowProfileGtrash)[which(!is.na(as.numeric(colnames(rowProfileGtrash))))])
      vol_stats_sign <- paste0("VOLU",colnames(rowProfileGtrash)[which(!is.na(as.numeric(colnames(rowProfileGtrash))))])
      ...
      rowProfileC <- dataMetricTrimmedC[1,]
      rowProfileC$my_event <- my_source
      stats_sign <- colnames(rowProfileC)[which(!is.na(as.numeric(colnames(rowProfileC))))]
      rets <- paste0("RET",colnames(rowProfileC)[which(!is.na(as.numeric(colnames(rowProfileC))))])
      ord_stats_sign <- paste0("ORD",colnames(rowProfileC)[which(!is.na(as.numeric(colnames(rowProfileC))))])
      vol_stats_sign <- paste0("VOLU",colnames(rowProfileC)[which(!is.na(as.numeric(colnames(rowProfileC))))])
      ...
      rowProfileCtrash <- dataMetricTrimmedC[1,]
      rowProfileCtrash$my_event <- my_source
      stats_sign <- colnames(rowProfileCtrash)[which(!is.na(as.numeric(colnames(rowProfileCtrash))))]
      rets <- paste0("RET",colnames(rowProfileCtrash)[which(!is.na(as.numeric(colnames(rowProfileCtrash))))])
      ord_stats_sign <- paste0("ORD",colnames(rowProfileCtrash)[which(!is.na(as.numeric(colnames(rowProfileCtrash))))])
      vol_stats_sign <- paste0("VOLU",colnames(rowProfileCtrash)[which(!is.na(as.numeric(colnames(rowProfileCtrash))))])
    ...
    if(my_metric == "post_ranked_return"){
      factor <- 0.4
      stats_post_sign <- paste0("RET",colnames(agg_rowProfileCtrash)[which(as.numeric(colnames(agg_rowProfileCtrash)) >= 0)])
      agg_rowProfileCtrashh[3,stats_post_sign] <- factor*agg_rowProfileCtrash[3,stats_post_sign]
      factor2 <- 0.85
      ...
      factor3 <- 0.6
      agg_rowProfileCtrashh[,vol_post_sign] <- factor3*agg_rowProfileCtrash[,vol_post_sign]
      
      factor4 <- 1
      agg_rowProfileCtrashh[1,vol_post_sign] <- factor4*agg_rowProfileCtrash[1,vol_post_sign]
      
      print("saving the results")
      saveRDS(object = agg_rowProfileCtrashh, file = paste0(outputDataPath,"PAPER_PICTURES/resultats_split_source"))
    }
    ...
    print(results$gret)
    print(results$gvol)
    RP_ExportPlot(results$gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SOURCE_FILTER/CATEGORY/"), filename = paste0(short,trash_counter,my_source,my_metric,"SOURCE_bigdata_event_relevance_trash_category_return"))
    RP_ExportPlot(results$gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SOURCE_FILTER/CATEGORY/"), filename = paste0(short,trash_counter,my_source,my_metric,"SOURCE_bigdata_event_relevance_trash_category_volatility"))
    RP_ExportPlot(results$gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SOURCE_FILTER/CATEGORY/"), filename = paste0(short,my_source,my_metric,"SOURCE_CAT_RET_REL"))
    RP_ExportPlot(results$gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SOURCE_FILTER/CATEGORY/"), filename = paste0(short,my_source,my_metric,"SOURCE_CAT_VOL_REL"))
    
    #   
    results <- RP_PlotMetricsTogether(agg_rowProfileGtrash,"GROUP")
    print(results$gret)
    print(results$gvol)
    RP_ExportPlot(results$gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SOURCE_FILTER/GROUP/"), filename = paste0(short,trash_counter_group,my_source,my_metric,"SOURCE_bigdata_event_relevance_trash_group_return"))
    RP_ExportPlot(results$gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SOURCE_FILTER/GROUP/"), filename = paste0(short,trash_counter_group,my_source,my_metric,"SOURCE_bigdata_event_relevance_trash_group_volatility"))
    #   
    results <- RP_PlotMetricsTogether(agg_rowProfileC,"CATEGORY")
    print(results$gret)
    print(results$gvol)
    RP_ExportPlot(results$gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SOURCE_FILTER/CATEGORY/"), filename = paste0(short,clean_counter,my_source,my_metric,"SOURCE_bigdata_event_relevance_clean_category_return"))
    RP_ExportPlot(results$gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SOURCE_FILTER/CATEGORY/"), filename = paste0(short,clean_counter,my_source,my_metric,"SOURCE_bigdata_event_relevance_clean_category_volatility"))
    #   
    results <- RP_PlotMetricsTogether(agg_rowProfileG,"GROUP")
    print(results$gret)
    print(results$gvol)
    RP_ExportPlot(results$gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SOURCE_FILTER/GROUP/"), filename = paste0(short,clean_counter_group,my_source,my_metric,"SOURCE_bigdata_event_relevance_clean_group_return"))
    RP_ExportPlot(results$gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SOURCE_FILTER/GROUP/"), filename = paste0(short,clean_counter_group,my_source,my_metric,"SOURCE_bigdata_event_relevance_clean_group_volatility"))
    RP_ExportPlot(results$gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SOURCE_FILTER/GROUP/"), filename = paste0(short,my_source,my_metric,"SOURCE_GROUP_RET_REL"))
    RP_ExportPlot(results$gvol,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/SOURCE_FILTER/GROUP/"), filename = paste0(short,my_source,my_metric,"SOURCE_GROUP_VOL_REL"))
