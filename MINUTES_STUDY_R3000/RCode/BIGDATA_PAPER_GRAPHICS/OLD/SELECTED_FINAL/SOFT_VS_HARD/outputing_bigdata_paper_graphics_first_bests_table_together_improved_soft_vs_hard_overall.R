...
writeJoinedHTMLPictures <- function(data, file){
...
hard_groups <- flagged_group[flagged_group$SOFT == "no",]
soft_groups <- flagged_group[flagged_group$SOFT == "yes",]
#################### end of flag group data
########################################
########################################
########################################
...
for (my_source in c("ALL")){
  ...
  for(my_metric in my_metrics){
    ...
    tableToSaveHard <- tableToSave[tableToSave$my_event %in% hard_groups$GROUP,]
    tableToSaveSoft <- tableToSave[tableToSave$my_event %in% soft_groups$GROUP,]
    
    tableToSaveHard$my_event <- firstup(tableToSaveHard$my_event)
    tableToSaveHard$my_event <- gsub("-"," ",tableToSaveHard$my_event)
    
    tableToSaveHard$volatility_correction <- five_stars(tableToSaveHard$volatility_correction)
    tableToSaveHard$post_ranked_return <- five_stars(tableToSaveHard$post_ranked_return)
    tableToSaveHard$card_post_return <- five_stars(tableToSaveHard$card_post_return )
    
    tableToSaveHard <- tableToSaveHard[order(tableToSaveHard$RANKING,decreasing = TRUE),]
    if(my_metric == "card_post_return"){
      tableToSaveHard <- tableToSave[order(tableToSaveHard$card_post_return,decreasing = TRUE),]
    }
    
    tableToSaveHard$RANKING <- NULL
    colnames(tableToSaveHard) <- 
      c(
        "GROUP",
        # "ENTITY",        
        # "EVENT",   
    ...
    rownames(tableToSaveHard) <- c()
    # tableToSave$SIMILARITY <- 1
    
    print(dim(tableToSaveHard))
    print(head(tableToSaveHard))
    

    writeHTMLPictures(data = tableToSaveHard,file=paste0(outputDataPath,"PAPER_PICTURES/TABLES_SOFT_HARD/",my_source,my_metric,"hard_bigdata_best_groups_html.png"))
    
    
    
    tableToSaveSoft$my_event <- firstup(tableToSaveSoft$my_event)
    tableToSaveSoft$my_event <- gsub("-"," ",tableToSaveSoft$my_event)
    
    tableToSaveSoft$volatility_correction <- five_stars(tableToSaveSoft$volatility_correction)
    tableToSaveSoft$post_ranked_return <- five_stars(tableToSaveSoft$post_ranked_return)
    tableToSaveSoft$card_post_return <- five_stars(tableToSaveSoft$card_post_return )
    
    tableToSaveSoft <- tableToSaveSoft[order(tableToSaveSoft$RANKING,decreasing = TRUE),]
    if(my_metric == "card_post_return"){
      tableToSaveSoft <- tableToSave[order(tableToSaveSoft$card_post_return,decreasing = TRUE),]
    }
    
    tableToSaveSoft$RANKING <- NULL
    colnames(tableToSaveSoft) <- 
      c(
        "GROUP",
        # "ENTITY",        
        # "EVENT",   
    ...
    rownames(tableToSaveSoft) <- c()
    # tableToSave$SIMILARITY <- 1
    
    print(dim(tableToSaveSoft))
    print(head(tableToSaveSoft))
    
    
    writeHTMLPictures(data = tableToSaveSoft,file=paste0(outputDataPath,"PAPER_PICTURES/TABLES_SOFT_HARD/",my_source,my_metric,"soft_bigdata_best_groups_html.png"))

  
    ################
    ################ Writing a joint table
    print(head(tableToSaveHard))
    print(head(tableToSaveSoft))
    joinedTable <- data.frame(Hard = tableToSaveHard$GROUP[c(1,2,3,4,5,8,9,6,7,10)],Soft = tableToSaveSoft$GROUP[1:table_size_limit])
    print(head(joinedTable))
    writeJoinedHTMLPictures(data = joinedTable,file=paste0(outputDataPath,"PAPER_PICTURES/TABLES_SOFT_HARD/",my_source,my_metric,"joined_bigdata_best_groups_html.png"))
    write.csv(x =  joinedTable,file=paste0(outputDataPath,"PAPER_PICTURES/TABLES_SOFT_HARD/",my_source,my_metric,"joined_bigdata_best_groups_html.csv"),row.names = FALSE)
