...
dataG <- dataG[dataG$aggregate_criteria == "GROUP",]
...
for(my_metric in my_metrics){
  ...
  for (my_softness in c(FALSE,TRUE)){
    ...
    if(my_metric == "card_post_ranked_return"){
      ...
      data$RANKING <- card_significance*data$post_ranked_return
      data$card_post_ranked_return <-   data$RANKING
    } else {
      data$RANKING <- data[,my_metric]
    }
    
    
    dataMetricTrimmedG  <- ddply(.data = data,.variables = "my_event",.fun = function(x){trimBest(x)})
    print("after")
    print(dim(dataMetricTrimmedG))
    ...
    writeHTMLPictures(data = tableToSave,file=paste0(outputDataPath,"PAPER_PICTURES/TABLES_SOFT_HARD/",my_source,my_softness,my_metric,"bigdata_best_groups_html.png"))
    
    tableToSave <- tableToSave[1:table_size_limit,]
    writeHTMLPictures(data = tableToSave,file=paste0(outputDataPath,"PAPER_PICTURES/TABLES_SOFT_HARD/",my_source,my_softness,my_metric,"clean_bigdata_best_groups_html.png"))
