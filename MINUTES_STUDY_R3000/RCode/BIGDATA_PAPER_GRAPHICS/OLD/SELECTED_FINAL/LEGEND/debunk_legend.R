...
#   colnames(DataFrame) <- c("MINUTES","+ SCHEDULED","+ UNSCHEDULED","- SCHEDULED","- UNSCHEDULED")
  ...
  g <- ggplot(DataFrame, aes(x=MINUTES, group=variable ,y=value, colour = variable, fill = variable))+
    ...
    # scale_color_manual(values=c("#619CFF","#00BA38","#619CFF","#00BA38")) +  
...
print(gret)
print(gvol)
RP_ExportPlot(gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/LEGEND/"), filename = paste0("LLEGEMDSCH"))
