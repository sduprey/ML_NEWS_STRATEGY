...
RP_PlotCIIntervalSuperposed <- function(DataFrameOne, DataFrameTwo, DataFrameThree, Title = "", names, FullExportingPath=NULL, type = 'RET') {
  ...
  #   colnames(DataFrame) <- c("MINUTES","+ SOFT","+ HARD","- SOFT","- HARD")
  
  DataFrame <- DataFrame[,c("MINUTES","FREQUENCY","RETURN")]
  colnames(DataFrame) <- c("MINUTES","HARD","SOFT")
  # colnames(DataFrame) <- c("MINUTES","UNSCHEDULED","SCHEDULED")
...
print(gret)
print(gvol)
# RP_ExportPlot(gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/CONFIDENCE/"), filename = paste0("LLEGEMDSCHED"))
RP_ExportPlot(gret,outputDataPath =paste0(outputDataPath,"PAPER_PICTURES/CONFIDENCE/"), filename = paste0("LLEGEMDSOFT"))
