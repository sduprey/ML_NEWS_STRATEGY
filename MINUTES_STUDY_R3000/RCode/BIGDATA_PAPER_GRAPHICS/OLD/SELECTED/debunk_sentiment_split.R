...
load(file = paste0(outputDataPath, "dualSentSplit.RData"))

sentiment<-"POSITIVE"
rowProfilePos <- whole_results[sentiment][1]$POSITIVE
sentiment <- "NEGATIVE" 
rowProfileNeg <- whole_results[sentiment][1]$NEGATIVE
results <- RP_PlotProfileTogether(rowProfilePos,rowProfileNeg)
RP_ExportPlot(results$gret,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/CONFIDENCE_SPLIT/best_split_bigdata_returns_category_average_profile_ci"))
print(results$gret)
