...
outputGraphicsBestProfileCITogether <- function(  dataNeg,dataPos,Russell_version = "R1000", type = "RET"){
  ...
  g2 <- RP_PlotCIIntervalSuperposedSplit(DataFramePos =  toplotDFPos,DataFrameNeg =  toplotDFNeg, Title = MyTitle, FullExportingPath =  NULL)
  return(g2)
}


RP_PlotCIIntervalSuperposedSplit <- function(DataFramePos, DataFrameNeg, Title = "", FullExportingPath=NULL) {
...
for(my_metric in my_metrics){
  ...
  for (my_schedulity in c(FALSE,TRUE)){
    ...
    ###########3
    sch_results_group[[as.character(my_schedulity)]] <- rowProfile
    ...
    sch_results[[as.character(my_schedulity)]] <- rowProfile
    ###########3
  ...
  schedulity<-"TRUE"
  rowProfilePos <- sch_results[schedulity][1]$`TRUE`
  schedulity <- "FALSE" 
  rowProfileNeg <- sch_results[schedulity][1]$`FALSE`
...
#   rowProfileNeg[sapply(rowProfileNeg,is.numeric)] <- factor*rowProfileNeg[sapply(rowProfileNeg,is.numeric)]
  ...
  RP_ExportPlot(results$gret,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/SCHEDULED_SPLIT/",my_metric,short,"_clean_split_bigdata_returns_category_average_profile_ci"))
  print(results$gret)
...
rowProfilePos <- whole_results[schedulity][1]$`POSITIVETRUE`
schedulity <- "POSITIVEFALSE" 
rowProfileNeg <- whole_results[schedulity][1]$`POSITIVEFALSE`

# factor <- 2.5/abs(rowProfilePos[2,"RET80"])
# # factor <- 1
# print("pos factor")
# print(factor)
# 
...
results <- RP_PlotProfileTogether(rowProfilePos,rowProfileNeg)
RP_ExportPlot(results$gret,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/SCHEDULED_SPLIT/",short,trash_counter,"_clean_split_bigdata_returns_category_average_profile_ci"))
print(results$gret)
