...
for(my_metric in my_metrics){
  ...
  for (my_softness in c(FALSE,TRUE)){
    for (my_sentiment in c("NEGATIVE","POSITIVE")){
      ...
      #       
      #       ###########3
      #       soft_results_group[[paste0(my_sentiment,as.character(my_softness))]] <- rowProfile
      #       
      #       
      ...
      soft_results[[paste0(my_sentiment,as.character(my_softness))]] <- rowProfile
      ###########3
  ...
  # save(soft_results, file = paste0(outputDataPath, "dualSentSchedule.RData"))
  
  softness<-"POSITIVETRUE"
  rowProfilePos <- soft_results[softness][1]$`POSITIVETRUE`
  softness <- "POSITIVEFALSE" 
  rowProfileNeg <- soft_results[softness][1]$`POSITIVEFALSE`
  results <- RP_PlotProfileTogether(rowProfilePos,rowProfileNeg)
  RP_ExportPlot(results$gret,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/SOFT_SPLIT/",my_metric,"positive_split_sent_bigdata_returns_category_average_profile_ci"))
  print(results$gret)
  
  
  
  softness<-"NEGATIVETRUE"
  rowProfilePos <- soft_results[softness][1]$`NEGATIVETRUE`
  softness <- "NEGATIVEFALSE" 
  rowProfileNeg <- soft_results[softness][1]$`NEGATIVEFALSE`
  results <- RP_PlotProfileTogether(rowProfilePos,rowProfileNeg)
  RP_ExportPlot(results$gret,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/SOFT_SPLIT/",my_metric,"negative_split_sent_bigdata_returns_category_average_profile_ci"))
  print(results$gret)
  ...
  if (my_metric == "card_post_ranked_return"){
    softness<-"POSITIVETRUE"
    rowProfilePOSITIVETRUE <- soft_results[softness][1]$`POSITIVETRUE`
    softness <- "POSITIVEFALSE" 
    rowProfilePOSITIVEFALSE <- soft_results[softness][1]$`POSITIVEFALSE`
    
  
    softness<-"NEGATIVETRUE"
    rowProfileNEGATIVETRUE <- soft_results[softness][1]$`NEGATIVETRUE`
    softness <- "NEGATIVEFALSE" 
    rowProfileNEGATIVEFALSE <- soft_results[softness][1]$`NEGATIVEFALSE`
...
#     print(results$gret)
    ...
    factor <- -1
    rowProfilePOSITIVETRUE[sapply(rowProfilePos,is.numeric)] <- factor*rowProfilePOSITIVETRUE[sapply(rowProfilePos,is.numeric)]
    rowProfilePOSITIVEFALSE[sapply(rowProfileNeg,is.numeric)] <- factor*rowProfilePOSITIVEFALSE[sapply(rowProfileNeg,is.numeric)]
    rowProfileNEGATIVETRUE[sapply(rowProfilePos,is.numeric)] <- factor*rowProfileNEGATIVETRUE[sapply(rowProfilePos,is.numeric)]
    rowProfileNEGATIVEFALSE[sapply(rowProfileNeg,is.numeric)] <- factor*rowProfileNEGATIVEFALSE[sapply(rowProfileNeg,is.numeric)]
    
    
    whole_results[["NEGATIVETRUE"]] = rowProfilePOSITIVETRUE
    whole_results[["NEGATIVEFALSE"]] = rowProfilePOSITIVEFALSE
    
    whole_results[["POSITIVETRUE"]] = rowProfileNEGATIVETRUE
    whole_results[["POSITIVEFALSE"]] = rowProfileNEGATIVEFALSE
...
print("Saving the all structures")
save(whole_results, file = paste0(outputDataPath, "dualSentSoft.RData"))
