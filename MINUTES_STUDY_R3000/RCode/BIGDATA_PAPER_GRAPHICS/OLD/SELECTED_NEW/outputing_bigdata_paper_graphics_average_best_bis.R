...
# source("./RCode/RP_PlottingUtils.R")
...
RP_PlotCIInterval <- function(DataFrame, Title = "", FullExportingPath=NULL) {
  ...
  ############# not when weighted
#   if (Title == "Abnormal returns"){
#     g <- g + scale_y_continuous(breaks=c(-125,-100,-75,-50,-25,0,25,50,75,100,125,150,175,200,225,250,275,300,325,350,375))
#   }
#   
#   if(trash_counter < 50){
#     g <- g + scale_y_continuous(breaks=c(-200,-100,0,100,200,300,400,500,600))
#   }
  ##############
  #############3
...
outputGraphicsBestProfileCI <- function( data,  Russell_version = "R1000", type = "RET"){
  ...
  if(type =="RET"){
    ...
    toplotDF <- toplotDF*5
    toplotDF$MINUTES <- toplotDF$MINUTES/5
    
    if(short == "short"){
      
      
      #       toplotDF$CI_LOW <- 3/4*( toplotDF$CI_LOW) + 1/4*(mav(toplotDF$CI_LOW))
      #       toplotDF$CI_HIGH <- 3/4*(toplotDF$CI_HIGH) + 1/4*mav(toplotDF$CI_HIGH)
      #       toplotDF$MEAN <- 3/4*(toplotDF$MEAN) + 1/4*mav(toplotDF$MEAN)
      #       toplotDF <- toplotDF[complete.cases(toplotDF),]
      toplotDF <- toplotDF[abs(toplotDF$MINUTES) <=90,]
      toplotDF <- toplotDF[toplotDF$MINUTES >=  -50,]
    } else {
      #       toplotDF$CI_LOW <- 3/4*( toplotDF$CI_LOW) + 1/4*(mav(toplotDF$CI_LOW))
      #       toplotDF$CI_HIGH <- 3/4*(toplotDF$CI_HIGH) + 1/4*mav(toplotDF$CI_HIGH)
      #       toplotDF$MEAN <- 3/4*(toplotDF$MEAN) + 1/4*mav(toplotDF$MEAN)
...
#   RP_ExportPlot(results$gret,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/BEST_PROFILES_WEIGHTED/",my_metric,"bigdata_returns_group_average_profile_ci"))
#   print(results$gret)
#   RP_ExportPlot(results$gvol,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/BEST_PROFILES_WEIGHTED/",my_metric,"bigdata_volatility_group_average_profile_ci"))
#   print(results$gvol)  
#   RP_ExportPlot(results$gvolu,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/BEST_PROFILES_WEIGHTED/",my_metric,"bigdata_volume_group_average_profile_ci"))
#   print(results$gvolu)  
#   RP_ExportPlot(results$gstats,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/BEST_PROFILES_WEIGHTED/",my_metric,"bigdata_significance_group_average_profile_ci"))
#   print(results$gstats)  
#   RP_ExportPlot(results$gstatscombi,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/BEST_PROFILES_WEIGHTED/",my_metric,"bigdata_significance_group_average_profile_ci"))
#   print(results$gstatscombi)  
#   
#   
...
#   #   rowProfile[4,] <- rowProfileSTATS[2,]
  ...
  dataCategoryBest <- dataCategoryBest[1:min(trash_counter, dim(dataCategoryBest)[1]),]
  ...
  ######################
  ######################
  ###################### weighting our returns
  dataCategoryBest$weighted_count <- dataCategoryBest$event_number_event_filtering/sum(dataCategoryBest$event_number_event_filtering)
  
  my_df <- dataCategoryBest$event_number_event_filtering*dataCategoryBest[,allNumericColumns]/sum(dataCategoryBest$event_number_event_filtering)
  
  my_diff_df <- differentiateRets(dataCategoryBest[,allRetsColumns])
  my_diff_df <- dataCategoryBest$event_number_event_filtering*my_diff_df/sum(dataCategoryBest$event_number_event_filtering)
  ...
  #     whole_results[["POSITIVE"]] = rowProfile
  #   }
  #   
  #   if(my_metric == "post_ranked_return"){
  #     toStore <- rowProfile
  #     factor <- -1
  #     toStore[sapply(toStore,is.numeric)] <- factor*toStore[sapply(toStore,is.numeric)]
  #     whole_results[["NEGATIVE"]] = toStore
  #   }
  #   
  #   
  ...
  print("outputing an average graphic for our metric group")
  results <- RP_PlotProfile(rowProfile)
  RP_ExportPlot(results$gret,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/BEST_PROFILES_WEIGHTED/",filter,short,trash_counter,my_metric,"bigdata_returns_category_average_profile_ci"))
  print(results$gret)
  RP_ExportPlot(results$gvol,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/BEST_PROFILES_WEIGHTED/",filter,short,trash_counter,my_metric,"bigdata_volatility_category_average_profile_ci"))
  print(results$gvol)  
  RP_ExportPlot(results$gvolu,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/BEST_PROFILES_WEIGHTED/",filter,short,trash_counter,my_metric,"bigdata_volume_category_average_profile_ci"))
  print(results$gvolu)  
  RP_ExportPlot(results$gstats,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/BEST_PROFILES_WEIGHTED/",filter,short,trash_counter,my_metric,"bigdata_significance_category_average_profile_ci"))
  print(results$gstats) 
  ##########################
  ##########################
  ##########################
...
save(whole_results, file = paste0(outputDataPath, "dualSentSplit.RData"))
