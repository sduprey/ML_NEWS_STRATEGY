...
for (my_source in c("WEB_NON_PREMIUM")){
  ...
  for(my_metric in my_metrics){
    ...
    for (my_schedulity in c(FALSE,TRUE)){
      ...
      library("boot")
      data <- dataGS[dataGS$SCHEDULED == my_schedulity,]
    ...
    RP_ExportPlot(results$gret,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/SCHEDULED_SPLIT/",my_source,my_metric,short,trash_counter,"_clean_split_bigdata_returns_category_average_profile_ci"))
    print(results$gret)
