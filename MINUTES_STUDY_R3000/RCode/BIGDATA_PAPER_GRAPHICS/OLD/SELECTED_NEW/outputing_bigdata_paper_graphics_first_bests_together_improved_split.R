...
outputGraphicsBestProfileStats <- function(product_criteria,aggregate_criteria,sentiment_criteria,similarity_gap_filter,ens_filter,event_number_event_filtering, gics_sector, my_event, localSource, dataFrame,  plotInArborescence, Russell_version = "R1000", Together = FALSE, ylim = FALSE){
  ...
  # g <- portMultiplePlot(g1,g2, plotlist = NULL, filename = NULL, outputDataPath = NULL, cols = 1, width = 10, height = 15)
  return(g1)
}
...
RP_PlotProfileTogetherVol <- function(rowProfilePos,rowProfileNeg, type = "VOLA"){
  ...
  gret <- outputGraphicsBestProfileCITogether(dataframeNeg,dataframePos,Russell_version = "R1000", type = type)
...
RP_PlotProfileTogetherStats <- function(rowProfilePos,rowProfileNeg){
  ...
  # dataframe <- as.data.frame(t(rbind(stats_sign,ord_stats_sign,vol_stats_sign,vola_stats_sign,rets)))
  dataframePos <- as.data.frame(t(rbind(stats_sign,vol_stats_sign,vola_stats_sign,rets)))
  
  colnames(dataframePos) <- c("CORRADO_CI_LOW","CORRADO","CORRADO_CI_HIGH","VOLUME_CI_LOW","VOLUME","VOLUME_CI_HIGH","VOLATILITY_CI_LOW","VOLATILITY","VOLATILITY_CI_HIGH","RETS_CI_LOW","RETS","RETS_CI_HIGH")
  dataframePos$MINUTES <- as.numeric(colnames(rowProfilePos)[which(!is.na(as.numeric(colnames(rowProfilePos))))])
  dataframePos <- dataframePos[,c("MINUTES","CORRADO_CI_LOW","CORRADO","CORRADO_CI_HIGH","VOLUME_CI_LOW","VOLUME","VOLUME_CI_HIGH","VOLATILITY_CI_LOW","VOLATILITY","VOLATILITY_CI_HIGH","RETS_CI_LOW","RETS","RETS_CI_HIGH")]
  ...
  # dataframe <- as.data.frame(t(rbind(stats_sign,ord_stats_sign,vol_stats_sign,vola_stats_sign,rets)))
  dataframeNeg <- as.data.frame(t(rbind(stats_sign,vol_stats_sign,vola_stats_sign,rets)))
  
  colnames(dataframeNeg) <- c("CORRADO_CI_LOW","CORRADO","CORRADO_CI_HIGH","VOLUME_CI_LOW","VOLUME","VOLUME_CI_HIGH","VOLATILITY_CI_LOW","VOLATILITY","VOLATILITY_CI_HIGH","RETS_CI_LOW","RETS","RETS_CI_HIGH")
  dataframeNeg$MINUTES <- as.numeric(colnames(rowProfileNeg)[which(!is.na(as.numeric(colnames(rowProfileNeg))))])
  dataframeNeg <- dataframeNeg[,c("MINUTES","CORRADO_CI_LOW","CORRADO","CORRADO_CI_HIGH","VOLUME_CI_LOW","VOLUME","VOLUME_CI_HIGH","VOLATILITY_CI_LOW","VOLATILITY","VOLATILITY_CI_HIGH","RETS_CI_LOW","RETS","RETS_CI_HIGH")]
  
  
  gret <- outputGraphicsBestProfileCITogether(dataframeNeg,dataframePos,Russell_version = "R1000", type = "STATS")
  #   gvol <- outputGraphicsBestProfileCITogether(dataframeNeg,dataframePos,Russell_version = "R1000", type = "VOL")
  #   gvolu <- outputGraphicsBestProfileCITogether(dataframeNeg,dataframePos,Russell_version = "R1000", type = "VOLU")
...
outputGraphicsBestProfileCITogether <- function(  dataNeg,dataPos,Russell_version = "R1000", type = "RET"){
  ...
  if (type == "VOL"){
    ...
    translatingFactor <- (1.2-min(toplotDFNeg[,-1]))
    toplotDFNeg <- toplotDFNeg+translatingFactor
    toplotDFNeg$MINUTES <- toplotDFNeg$MINUTES - translatingFactor
    
    
    translatingFactor <- (1.2-min(toplotDFPos[,-1]))
    toplotDFPos <- toplotDFPos+translatingFactor
    toplotDFPos$MINUTES <- toplotDFPos$MINUTES - translatingFactor
  ...
  if(type =="RET"){
    ...
    MyTitle <- "Abnormal returns"
    toplotDFPos <- toplotDFPos*5
    toplotDFPos$MINUTES <- toplotDFPos$MINUTES/5
    toplotDFNeg <- toplotDFNeg*5
    toplotDFNeg$MINUTES <- toplotDFNeg$MINUTES/5
  ...
  if(type =="STATS"){
    toplotDFPos<- dataPos[,c("MINUTES","CORRADO_CI_LOW","CORRADO","CORRADO_CI_HIGH")]
    colnames(toplotDFPos) <- c("MINUTES","CI_LOW","MEAN","CI_HIGH")
    
    toplotDFNeg<- dataNeg[,c("MINUTES","CORRADO_CI_LOW","CORRADO","CORRADO_CI_HIGH")]
    colnames(toplotDFNeg) <- c("MINUTES","CI_LOW","MEAN","CI_HIGH")
    
    MyTitle <- "Abnormal significance"
    translatingFactor <- 0.075
    toplotDFNeg <- toplotDFNeg+translatingFactor
    toplotDFNeg$MINUTES <- toplotDFNeg$MINUTES - translatingFactor
...
RP_PlotCIIntervalSuperposed <- function(DataFramePos, DataFrameNeg, Title = "", FullExportingPath=NULL) {
  ...
  ylabelText <- "BPS cumulated minute return"
  if (Title == "Abnormal volatility"){
    ylabelText <- "Abnormal ratio"
  }
  ...
  if (Title == "Abnormal significance"){
    g <- g + scale_y_continuous(breaks=c(0.7,0.8,0.9))
  }
  g <- g + geom_vline(aes(xintercept=0),colour = 'black', size = 1.5,linetype="dashed")
  if (!is.null(FullExportingPath)){
    RP_ExportPlot(g,FullExportingPath,"")
...
for(my_metric in my_metrics){
  ...
  sent_vol_results <- list(
    "POSITIVE"=NULL,
    "NEGATIVE"=NULL
  )
  
  sent_results_group <- list(
    "POSITIVE"=NULL,
    "NEGATIVE"=NULL
  )
  ...
  for (my_sentiment in sentiments){
    ...
    ###########3
    sent_results_group[[my_sentiment]] <- rowProfile
    ...
    sent_results[[my_sentiment]] <- rowProfile
    ###########3
  ...
  sentiment<-"POSITIVE"
  rowProfilePos <- sent_results[sentiment][1]$POSITIVE
  sentiment <- "NEGATIVE" 
  rowProfileNeg <- sent_results[sentiment][1]$NEGATIVE
  rowProfilePosP <- rowProfilePos
  rowProfileNegP <- rowProfileNeg
  traffic <-  "traf"
  # traffic <-  "traf"
  if (traffic == "not_traf")
  {
    results <- RP_PlotProfileTogether(rowProfilePosP,rowProfileNegP)
     print(results$gret)
    RP_ExportPlot(results$gret,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/CONFIDENCE_SPLIT/",traffic,short,trash_counter,my_metric,"split_bigdata_returns_category_average_profile_ci"))
    ...
    if(my_metric == "post_ranked_return"){
      ...
      rowProfilePosP[,stats_post_sign] <- factor*rowProfilePos[,stats_post_sign]
      rowProfileNegP[,stats_post_sign] <- factor*rowProfileNeg[,stats_post_sign]
    }
    ...
    if(my_metric == "card_post_ranked_return"){
      ...
      vol_post_sign <- paste0("VOLA",colnames(rowProfilePos)[which(!is.na(as.numeric(colnames(rowProfilePos)) ))])

      factor3 <- 0.9
      rowProfilePosP[,vol_post_sign] <- factor3*rowProfilePos[,vol_post_sign]
      rowProfileNegP[,vol_post_sign] <- factor3*rowProfileNeg[,vol_post_sign]
    }
    results <- RP_PlotProfileTogether(rowProfilePosP,rowProfileNegP)
    RP_ExportPlot(results$gret,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/CONFIDENCE_SPLIT/",traffic,short,trash_counter,my_metric,"split_bigdata_returns_category_average_profile_ci"))
    print(results$gret)
    RP_ExportPlot(results$gret,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/CONFIDENCE_SPLIT/",short,trash_counter,my_metric,"split_bigdata_returns_category_average_profile_ci"))
    
    
  } 

  
  results <- RP_PlotProfileTogetherVol(rowProfilePosP,rowProfileNegP,"VOL")
  RP_ExportPlot(results$gret,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/CONFIDENCE_SPLIT/",short,trash_counter,my_metric,"split_bigdata_volatility_category_average_profile_ci"))
  print(results$gret)
  
  
  results <- RP_PlotProfileTogetherVol(rowProfilePosP,rowProfileNegP,"VOLU")
  RP_ExportPlot(results$gret,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/CONFIDENCE_SPLIT/",short,trash_counter,my_metric,"split_bigdata_volume_category_average_profile_ci"))
  print(results$gret)
  
  
  results <- RP_PlotProfileTogetherStats(rowProfileNegP,rowProfilePosP)
  RP_ExportPlot(results$gret,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/CONFIDENCE_SPLIT/",short,trash_counter,my_metric,"split_bigdata_significance_category_average_profile_ci"))
  print(results$gret)
  
  
  
  #   sentiment<-"POSITIVE"
  #   rowProfilePos <- sent_results_group[sentiment][1]$POSITIVE
  #   sentiment <- "NEGATIVE" 
  #   rowProfileNeg <- sent_results_group[sentiment][1]$NEGATIVE
  #   results <- RP_PlotProfileTogether(rowProfilePos,rowProfileNeg)
  #   RP_ExportPlot(results$gret,outputDataPath = outputDataPath,filename = paste0("PAPER_PICTURES/CONFIDENCE_SPLIT/",short,trash_counter,my_metric,"split_bigdata_returns_group_average_profile_ci"))
  #   print(results$gret)
  #   
