...
all_data <- all_data[all_data$aggregate_criteria == "CATEGORY",]
...
for(my_metric in my_metrics){
  ...
  for (my_group in my_group_list){
    ...
    print(my_group)
    dataMetricTrimmedC <- dataMetricTrimmed[dataMetricTrimmed$GROUP == my_group,]
...
#     }
    ...
    results <- getPlotBestProfilesOneByOne("CATEGORY",dataMetricTrimmedC,all_group_events,all_category_events,my_metric)
    print(results$gret)
