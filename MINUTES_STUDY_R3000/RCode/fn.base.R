...
path.wd <- getwd()
# path.wd <- "C://\My_R_Workspace/BOSCH/"
...
fn.print.auc.err <- function(actual, pred, do.print = T) { 
  ...
  if (is.data.frame(actual)) {
    if (!is.null(actual$Response)) {
      actual <- actual$Response
    }  else if (!is.null(actual$Response)) {
      actual <- actual$Response
    }
  }
...
fn.print.matt.err <- function(actual, pred, do.print = T) { 
  ...
  if (is.null(actual) || all(is.na(actual))) {
    ...
    library("Metrics")
    matt <- computeErrorMatthew(actual,pred)
    df <- data.frame(Length = length(actual),
                     MATT = matt$maxo)
  }
  ...
  y.def = min(data.tr[[col.y]]))
{
  ...
  save(list = map.name, file = paste0(path.wd, "/",dir, "/", map.name, ".RData"))
  cat("done \n")
  toc()
...
fn.write.submission <- function(pred, file.name) { 
  ...
  pred <- data.frame(id = 1:NROW(pred), Response=pred)
  write.csv(pred,
  ...
  for (col.name in colnames(tr)) {
    if (col.name != "Response") {
      ...
      model.lme <- suppressWarnings(fn.train.lmer(
        formula= as.formula(paste0("Response ~ 1 + (1|", col.name, ")")),
        data = out.tr,
        family = binomial(),
        verbose = F))
...
fn.ens.load.df <- function(name, cols.ens) {
  ...
  ens.df <-  get(paste0("data.", name))[, "Response", drop = F]
