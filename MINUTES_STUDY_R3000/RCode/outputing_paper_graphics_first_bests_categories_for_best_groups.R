...
getPlotBestProfilesInsideGroup <- function(data,all_events,suffix,groupName){
  plotLimit <- 6
  data <- data[order(data$RANKING,decreasing = TRUE),]
  for (i in 1:min(plotLimit, dim(data)[1])){
    rowProfile <- data[i,]
  ...
  if(i == 4){
    gname <- ExportMultiplot(gret1,gvol1,
                             ...
                             gret3,gvol3,
                             gret4,gvol4,
                             plotlist = NULL, filename = paste0("bestInsideCategories",groupName,suffix), outputDataPath = paste0(outputDataPath,"PAPER_PICTURES/"), cols = 5, width = width, height = height)
...
MappingGroupCategory_RPData <- readRDS(file = paste0(outputDataPath,"RPNA_group_category.rds"))

data <- data[!grepl("bankruptcy",data$my_event),]
data <- data[!grepl("assets",data$my_event),]
data <- data[!grepl("technical",data$my_event),]

data <- data[!grepl("imbalance",data$my_event),]
data <- data[!grepl("assets",data$my_event),]
...
data <- data[!grepl("regulatory",data$my_event),]

data <- data[!grepl("stock-loss",data$my_event),]
data <- data[!grepl("stock-gain",data$my_event),]
data <- data[!grepl("stock-prices",data$my_event),]
data <- data[!grepl("war-conflict",data$my_event),]

data <- data[!grepl("taxes",data$my_event),]
data <- data[!grepl("transportation",data$my_event),]
data <- data[!grepl("security",data$my_event),]
data <- data[!grepl("civil-unrest",data$my_event),]


data <- data[!grepl("corporate-responsibility",data$my_event),]
data <- data[!grepl("crime",data$my_event),]
...
for(my_metric in my_metrics){
  ...
  bestMetricGroups <- dataMetricTrimmed[dataMetricTrimmed$aggregate_criteria == "GROUP",c("my_event",'RANKING')]
  bestMetricGroups <- bestMetricGroups[order(bestMetricGroups$RANKING,decreasing = TRUE),]
  
  bestMetricFiveGroups <- bestMetricGroups[1:6,]
  for(my_best_group in bestMetricFiveGroups$my_event){
    #### get the categories which belong to the group
    print(my_best_group)
    groupCategories <- MappingGroupCategory_RPData[as.character(MappingGroupCategory_RPData$group) == my_best_group,]
    groupCategories <- groupCategories[!is.na(groupCategories$group),]
    print("Associated categories")
    print(groupCategories$category)
    dataMetricTrimmedGroupBest <- dataMetricTrimmed[dataMetricTrimmed$my_event %in% groupCategories$category,]
    
    print("Displaying the affiliated categories")
    
    filename <- getPlotBestProfilesInsideGroup(dataMetricTrimmedGroupBest,groupCategories$category,my_metric,my_best_group)
    print(filename)
