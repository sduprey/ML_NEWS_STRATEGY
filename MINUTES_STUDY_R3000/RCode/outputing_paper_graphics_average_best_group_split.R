...
for(my_metric in my_metrics){
  ...
  my_df <- dataGroupPosBest[,allNumericColumns]
  my_diff_df <- differentiateRets(dataGroupPosBest[,allRetsColumns])
  ...
  my_df <- dataGroupNegBest[,allNumericColumns]
  my_diff_df <- differentiateRets(dataGroupNegBest[,allRetsColumns])
  ...
  RP_ExportPlot(results$gret,outputDataPath = outputDataPath,filename = "PAPER_PICTURES/split_returns_group_average_profile_ci")
  print(results$gret)
