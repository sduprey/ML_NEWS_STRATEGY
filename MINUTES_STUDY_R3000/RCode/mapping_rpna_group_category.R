...
mapping_group_category_request <- 'SELECT  distinct "GROUP", CATEGORY  FROM analytics.rpna_400_eqt eq group by "GROUP", CATEGORY'

dbCon<-RP_DatabaseConnect(user)
MappingGroupCategory_RPData <- dbGetQuery(dbCon, mapping_group_category_request)

RP_SaveDataFrame(dataframe = MappingGroupCategory_RPData,outputDataPath =outputDataPath ,filename = "RPNA_group_category")
