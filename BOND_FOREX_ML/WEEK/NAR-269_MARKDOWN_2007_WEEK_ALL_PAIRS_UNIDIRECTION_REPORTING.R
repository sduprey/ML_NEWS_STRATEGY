...
#   }
  for (nb in 1:nb_deep_solution){
    ...
    if (nb == nb_deep_solution){
      ...
      # SaveDataFrame(all_results$results,outputDataPathWeek,paste(my_pair[1], my_pair[2], biggest_today_IR$algorithm,"EU",biggest_today_IR$europe_as_third_country,"ZS",biggest_today_IR$zscore,"depth",biggest_today_IR$depth,"amp",biggest_today_IR$spread_amplification_factor,"inv_hor",biggest_today_IR$horizon,"roll_win",biggest_today_IR$rolling_window,"rec_freq",biggest_today_IR$recalibration_frequency,"unipred_spread_results_week_2007",sep=""))
      #     
      ...
      #         results_prediction <- tryCatch( readRDS(paste(outputDataPathWeek,my_result_prediction_spread_filename, sep = "")),
      ...
      my_result_prediction_spread_filename <- paste(my_result_prediction_spread_name, ".rds",sep="")
      all_results <- tryCatch( readRDS(paste(outputDataPathWeek,my_result_prediction_spread_filename, sep = "")),
                               error = function(e) {NULL})
...
SaveDataFrame(aggregated_df,outputDataPathStrategyWeek,"all_pairs_prediction_results_week_2007")
# 
# for (my_pair in my_pairs){
#   
#   week_us_de_pair_results <- all_pair_results[all_pair_results$second_leg == my_pair[2],]
