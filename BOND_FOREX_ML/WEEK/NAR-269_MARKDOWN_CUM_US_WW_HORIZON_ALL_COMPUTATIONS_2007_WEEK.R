...
options(warn=-1)
my_pair_IR_results_df_name <- "all_pairs_week_2007"
my_pair_IR_results_df <- data.frame(second_leg=character(),
                                    horizon=double(),
                                    rolling_window=double(),
                                    recalibration_frequency=double(),
...
for (my_pair in my_pairs){
  ...
  # my_depths <- c(3,4)
  my_depths <- c(3,4)
  my_algorithms <- c("xgboost_cv","xgboost_caret_cv","rpart_cv","rpart_pruned","xgboost","rpart","random_forest","rpart","svm")
  ...
  recalibration_frequencies <- 52*rev(rolling_windows)
            ...
            for (my_horizon in investment_horizon) {
              for (spread_amplification_factor in spread_amplification_factors){
                for(recalibration_frequency in recalibration_frequencies){
                  ...
                  #               # )
                  #               
                  #               SaveDataFrame(all_results$results,outputDataPath,paste(my_pair[1], my_pair[2], algorithm,"EU",europe_as_third_country,"ZS",zscore,"depth",depth,"amp",spread_amplification_factor,"inv_hor",my_horizon,"spread_results_2007",sep=""))
                  #               SaveDataFrame(all_results$sentiments,outputDataPath,paste(my_pair[1], my_pair[2], algorithm,"EU",europe_as_third_country,"ZS",zscore,"depth",depth,"amp",spread_amplification_factor,"inv_hor",my_horizon,"spread_sentiments_2007",sep=""))
                  ...
                  #               print(spread_amplification_factor)
                  #               
                  my_result_spread_name <- paste(my_pair[1], my_pair[2], algorithm,"EU",europe_as_third_country,"ZS",zscore,"depth",depth,"amp",spread_amplification_factor,"inv_hor",my_horizon,"roll_win",my_rolling_window,"rec_freq",recalibration_frequency,"spread_results_week_2007",sep="")
                  ...
                  if (!is.null(results)){
                    ...
                    new_row_df <- data.frame(second_leg=my_pair[2],
                                             horizon=my_horizon,
                                             rolling_window=my_rolling_window,
                    ...
                    colnames(my_pair_IR_results_df) <- c("second_leg","horizon","rolling_window","recalibration_frequency","spread_amplification_factor","depth", "algorithm", "europe_as_third_country", "zscore","IR_spreadbidir_today", "IR_spreadbidir_tomorrow", "IR_spreadbidir_yesterday" )
                    ...
                    #                 
                    #                 my_result_sentiment_spread_name <- paste(my_pair[1], my_pair[2], algorithm,"EU",europe_as_third_country,"ZS",zscore,"depth",depth,"amp",spread_amplification_factor,"inv_hor",my_horizon, "spread_sentiments_2007",sep="")
...
SaveDataFrame(my_pair_IR_results_df,outputDataPathStrategyWeek,my_pair_IR_results_df_name)

#### results

all_pair_results <- readRDS(paste(outputDataPathStrategyWeek,"all_pairs_week_2007.rds", sep = ""))
...
for (my_pair in my_pairs){
  ...
  for (nb in 1:3){
    max_today_IR <- which.max(my_pair_results$IR_spreadbidir_today)
...
#   my_pair_results <- all_pair_results[all_pair_results$second_leg == my_pair[2],]
#   for (nb in 1:3){
#     max_tomorrow_IR <- which.max(my_pair_results$IR_spreadbidir_tomorrow)
...
#     print("All")
#     print(biggest_today_IR)
#     week_us_de_pair_results <- my_pair_results[-max_tomorrow_IR,]
#   }
# }
...
# 
# my_pair = c("US","DE")
# my_pair_IR_results_df_name <- paste(my_pair[1], my_pair[2],"_2007", sep="")
# us_de_pair_results <- readRDS(paste(outputDataPath,paste(my_pair_IR_results_df_name,".rds",sep=""), sep = ""))
# colnames(us_de_pair_results)
# print(" First biggest today weekly IRs")
# week_us_de_pair_results <- us_de_pair_results[us_de_pair_results$horizon==21,]
# 
# 
# for (nb in 1:3){
...
#   print("All")
#   print(biggest_today_IR)
#   week_us_de_pair_results <- week_us_de_pair_results[-max_tomorrow_IR,]
# }
# 
