...
#   }
  for (nb in 1:nb_deep_solution){
    ...
    # max_today_IR <- which.max(abs(week_us_de_pair_results$IR_spreadbidir_today))
    max_today_IR <- which.max(abs(week_us_de_pair_results$IR_spreadbidir_today))
    max_tomorrow_IR <- which.max(abs(week_us_de_pair_results$IR_spreadbidir_tomorrow))
    
    biggest_today_IR <- week_us_de_pair_results[max_today_IR,]
    biggest_tomorrow_IR <- week_us_de_pair_results[max_tomorrow_IR,]
    
    
    if (abs(biggest_tomorrow_IR$IR_spreadbidir_tomorrow) > abs(biggest_today_IR$IR_spreadbidir_today) ){
      biggest_today_IR <- biggest_tomorrow_IR
      max_today_IR <-  max_tomorrow_IR
    }
    ...
    week_us_de_pair_results <- week_us_de_pair_results[-max_today_IR,]
    
    # we do recompute every thing
    # capture.output(
...
#     SaveDataFrame(all_results$first_leg,outputDataPathWeek,paste(my_pair[1], my_pair[2], biggest_today_IR$algorithm,"EU",biggest_today_IR$europe_as_third_country,"ZS",biggest_today_IR$zscore,"depth",biggest_today_IR$depth,"amp",biggest_today_IR$spread_amplification_factor,"inv_hor",biggest_today_IR$horizon,"roll_win",biggest_today_IR$rolling_window,"rec_freq",biggest_today_IR$recalibration_frequency,"spread_first_leg_week_2007",sep=""))
#     SaveDataFrame(all_results$second_leg,outputDataPathWeek,paste(my_pair[1], my_pair[2], biggest_today_IR$algorithm,"EU",biggest_today_IR$europe_as_third_country,"ZS",biggest_today_IR$zscore,"depth",biggest_today_IR$depth,"amp",biggest_today_IR$spread_amplification_factor,"inv_hor",biggest_today_IR$horizon,"roll_win",biggest_today_IR$rolling_window,"rec_freq",biggest_today_IR$recalibration_frequency,"spread_second_leg_week_2007",sep=""))
#     SaveDataFrame(all_results$features_names,outputDataPathWeek,paste(my_pair[1], my_pair[2], biggest_today_IR$algorithm,"EU",biggest_today_IR$europe_as_third_country,"ZS",biggest_today_IR$zscore,"depth",biggest_today_IR$depth,"amp",biggest_today_IR$spread_amplification_factor,"inv_hor",biggest_today_IR$horizon,"roll_win",biggest_today_IR$rolling_window,"rec_freq",biggest_today_IR$recalibration_frequency,"spread_features_week_2007",sep=""))
#     
#     SaveDataFrame(all_results$calibration_parameters,outputDataPathWeek,paste(my_pair[1], my_pair[2], biggest_today_IR$algorithm,"EU",biggest_today_IR$europe_as_third_country,"ZS",biggest_today_IR$zscore,"depth",biggest_today_IR$depth,"amp",biggest_today_IR$spread_amplification_factor,"inv_hor",biggest_today_IR$horizon,"roll_win",biggest_today_IR$rolling_window,"rec_freq",biggest_today_IR$recalibration_frequency,"spread_calibration_week_2007",sep=""))
#     
    ...
    calibration <- tryCatch( readRDS(paste(outputDataPathWeek,my_calibration_filename, sep = "")),
                             error = function(e) {NULL})
    
    my_features_name <- paste(my_pair[1], my_pair[2], biggest_today_IR$algorithm,"EU",biggest_today_IR$europe_as_third_country,"ZS",biggest_today_IR$zscore,"depth",biggest_today_IR$depth,"amp",biggest_today_IR$spread_amplification_factor,"inv_hor",biggest_today_IR$horizon,"roll_win",biggest_today_IR$rolling_window,"rec_freq",biggest_today_IR$recalibration_frequency,"spread_features_week_2007",sep="")
    my_features_filename <- paste(my_features_name, ".rds",sep="")
    
    features_names <- tryCatch( readRDS(paste(outputDataPathWeek,my_features_filename, sep = "")),
                                error = function(e) {NULL})
    ...
    first_leg <- tryCatch( readRDS(paste(outputDataPathWeek,my_first_leg_spread_filename, sep = "")),
                           error = function(e) {NULL})
    ...
    g<-ggplot(
      ...
      theme(legend.key.size = unit(0.7, "cm"))
    print(g)          
    ExportPlot(g,outputDataPathWeek,paste(my_result_spread_name,"bis"))
...
for (my_pair in my_pairs){
  ...
  for (nb in 1:nb_deep_solution){
    my_result_spread_name <- paste(my_result_spread_name_root, nb,sep="")
    # max_today_IR <- which.max(abs(week_us_de_pair_results$IR_spreadbidir_today))
    max_today_IR <- which.max(abs(week_us_de_pair_results$IR_spreadbidir_tomorrow))
