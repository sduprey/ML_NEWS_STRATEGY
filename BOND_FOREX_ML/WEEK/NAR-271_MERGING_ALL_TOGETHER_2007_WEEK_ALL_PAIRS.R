...
# 
# SaveDataFrame(all_results_df, outputDataPath, "all_pairs_results_week_2007")
...
IR_spreadbidir_today <- computeIR(all_pair_combined_results_12_28$DE_STRATEGY_RETURN, 5)
print(IR_spreadbidir_today)
IR_spreadbidir_today <- computeIR(all_pair_combined_results_12_28$GB_STRATEGY_RETURN, 5)
print(IR_spreadbidir_today)
IR_spreadbidir_today <- computeIR(all_pair_combined_results_12_28$JP_STRATEGY_RETURN, 5)
print(IR_spreadbidir_today)

IR_spreadbidir_today <- computeIR(all_pair_combined_results_01_01$DE_STRATEGY_RETURN, 5)
print(IR_spreadbidir_today)
IR_spreadbidir_today <- computeIR(all_pair_combined_results_01_01$GB_STRATEGY_RETURN, 5)
print(IR_spreadbidir_today)
IR_spreadbidir_today <- computeIR(all_pair_combined_results_01_01$JP_STRATEGY_RETURN, 5)
print(IR_spreadbidir_today)
...
results$STRATEGY_EQ_WEIGHTS_BOND <- (results$DE_FIRST_BOND + results$DE_SECOND_BOND+ results$GB_SECOND_BOND+ results$JP_SECOND_BOND)/4
...
# results$JP_WEIGHT <- results$JP_SECOND_WEIGHT/3
# results$GB_WEIGHT <- results$GB_SECOND_WEIGHT/3
colnames(results) <- c(old_columns,"STRATEGY_RETURN","STRATEGY_TODAY","US_WEIGHT","DE_WEIGHT","JP_WEIGHT","GB_WEIGHT","STRATEGY_EQ_WEIGHTS_BOND")



### Deutschland visualization
my_result_spread_name <- "US_DE_JP_GB"
my_pair[1]<-"US"
my_pair[2]<-"DE"
toplot_df <-  melt(results[,c("DATES", "STRATEGY_TODAY","STRATEGY_EQ_WEIGHTS_BOND")],"DATES")
my_title <- paste("Macro Sentiment ML over Ravenpack ESS metrics and taxonomy"," all",sep="")
g<-ggplot(
  toplot_df,aes(
...
SaveDataFrame(all_pair_combined_results_12_28, outputDataPathMergingWeek, "all_pairs_results_week_2007")
write.csv(all_pair_combined_results_12_28, file = paste(outputDataPathMergingWeek,"all_pairs_results_week_2007.csv",sep=""), row.names = FALSE)
