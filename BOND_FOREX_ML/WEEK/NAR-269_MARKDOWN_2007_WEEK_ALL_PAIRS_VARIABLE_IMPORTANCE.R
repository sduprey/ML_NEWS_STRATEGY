...
for (my_pair in my_pairs){
  ...
  for (nb in 1:nb_deep_solution){
    ...
    max_today_IR <- which.max(week_us_de_pair_results$IR_spreadbidir_today)
    ...
    second_leg <- tryCatch( readRDS(paste(outputDataPathWeek,my_second_leg_spread_filename, sep = "")),
                           error = function(e) {NULL})
    ...
    features_names <- tryCatch( readRDS(paste(outputDataPathWeek,my_features_spread_filename, sep = "")),
                           error = function(e) {NULL})
...
#           #           print(imp_plot)
#           #           ExportPlot(imp_plot,outputDataPathWeek,paste0("first",my_pair[2],nb_trees,"histogram"))
#           
#           tree_plot <- xgb.plot.tree(feature_names = features_names, model = my_model)
#           print(tree_plot)
#           # does not work because this is a html widget
#           # ExportPlot(tree_plot,outputDataPathWeek,paste0("first",my_pair[2],nb_trees,"tree"))
#           
#           nb_trees <- nb_trees + 1
#         }
#       }
...
#         #     summary(tree_one)
#         #     
#         #     png(paste(outputDataPath,paste("My_tree_pictures/",my_pair[2],"_week_2007_tree.png",sep=""),sep=""), width = 1200, height = 800)
