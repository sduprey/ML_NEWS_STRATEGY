...
for (my_pair in my_pairs){
  ...
  for (nb in 1:nb_deep_solution){
    ...
    # max_today_IR <- which.max((-month_us_de_pair_results$IR_spreadbidir_tomorrow))
...
#     SaveDataFrame(all_results$calibration_parameters,outputDataPathMonth,paste(my_pair[1], my_pair[2], biggest_today_IR$algorithm,"EU",biggest_today_IR$europe_as_third_country,"ZS",biggest_today_IR$score,"depth",biggest_today_IR$depth,"amp",biggest_today_IR$spread_amplification_factor,"inv_hor",biggest_today_IR$horizon,"roll_win",biggest_today_IR$rolling_window,"rec_freq",biggest_today_IR$recalibration_frequency,"spread_calibration_month_2007",sep=""))
