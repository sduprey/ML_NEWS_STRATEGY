...
#     
    ...
    if (nb == nb_deep_solution){
      ...
      # we take the reversal 
      if(IR_spreadbidir_tomorrow <= 0){
        results$STRATEGY_TOMORROW <- - results$STRATEGY_TOMORROW
      } 
...
# results$STRATEGY_RETURN <- (results$JP_STRATEG+results$GB_STRATEGY_RETURN+results$DE_STRATEGY_RETURN)/3
