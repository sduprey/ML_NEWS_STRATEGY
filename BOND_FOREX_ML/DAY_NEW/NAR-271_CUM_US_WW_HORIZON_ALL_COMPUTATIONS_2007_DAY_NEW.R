...
source("./RCode/RP_Day_Spread_Utils.R")
...
for (my_pair in my_pairs){
  ...
  # my_depths <- c(3,4)
  my_depths <- c(3)
  # my_algorithms <- c("xgboost_cv","rpart_cv","rpart_pruned","xgboost","rpart","random_forest","svm")
  ...
  recalibration_frequencies <- 12*10
            ...
            for (my_horizon in investment_horizon) {
              for (spread_amplification_factor in spread_amplification_factors){
                for(recalibration_frequency in recalibration_frequencies){
                  # capture.output(
                  all_results <- compute_day_spread_strategy_west_first_horizon(inputDataPath, outputDataPath, my_pair[1], my_pair[2], backtesting_starting_date, backtesting_ending_date, spread_amplification_factor,algorithm, europe_as_third_country, zscore, depth, my_horizon, my_rolling_window, recalibration_frequency)
                  # )
