...
compute_day_spread_strategy_west_first_horizon <- function(inputDataPath, outputDataPath, first_country, second_country, backtesting_starting_date, backtesting_ending_date, spread_amplification_factor, algorithm_used, europe_as_third_country, zscore, depth, investment_horizon, rolling_window, recalibration_frequency){
  ...
  for (my_country in my_countries){
    ...
    my_relevant_columns <- c("DATE","macro_key","rp_entity_id","ess","ens","ens_similarity_gap")#,"type","sub_type","news_type")
    my_to_average_columns <- c("ess","ens","ens_similarity_gap")#,"type","news_type")
    ...
    daily_global_macroRPData <- aggregate(global_macroRPData[,my_to_average_columns], by=list(global_macroRPData$DATE,global_macroRPData$macro_key,global_macroRPData$rp_entity_id),FUN=mean)
    ...
    daily_global_macroRPData<-dcast(daily_global_macroRPData, "DATE ~ macro_key+rp_entity_id")
    
    # daily_global_macroRPData <- aggregate(global_macroRPData[,my_to_average_columns], by=list(global_macroRPData$DATE,global_macroRPData$rp_entity_id,global_macroRPData$country_code,global_macroRPData$macro_key), stat)
    
    # daily_global_macroRPData <- aggregate(global_macroRPData[,my_to_average_columns], by=list(global_macroRPData$DATE,global_macroRPData$macro_key), FUN=mean)
#     ddply(global_macroRPData,.(global_macroRPData$DATE,global_macroRPData$macro_key,global_macroRPData$rp_entity_id),stat)
#     
#     # Ravenpack identity predictor also
#     global_macroRPData[,unique(global_macroRPData$rp_entity_id)] <- NA
#     

#     daily_global_macroRPData[daily_global_macroRPData$DATE == "2014-09-23",]
#     daily_global_macroRPData[daily_global_macroRPData$DATE == "2015-09-03",]
#     
#     
