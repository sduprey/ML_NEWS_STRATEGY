...
#     my_result_spread_name <- paste(my_pair[1], my_pair[2], biggest_today_IR$algorithm,"EU",biggest_today_IR$europe_as_third_country,"ZS",biggest_today_IR$zscore,"depth",biggest_today_IR$depth,"amp",biggest_today_IR$spread_amplification_factor,"inv_hor",my_horizon, "spread_results_week_2007",sep="")
...
all_pair_combined_results <- readRDS(paste(outputDataPath,"all_pairs_results_month_2007.rds", sep = ""))
# FACT CHECKING

DE_IR_spreadbidir_today <- computeIR(all_pair_combined_results$DE_STRATEGY_RETURN, 21)
print(DE_IR_spreadbidir_today)
GB_IR_spreadbidir_today <- computeIR(all_pair_combined_results$GB_STRATEGY_RETURN, 21)
print(GB_IR_spreadbidir_today)
JP_IR_spreadbidir_today <- computeIR(all_pair_combined_results$JP_STRATEGY_RETURN, 21)
print(JP_IR_spreadbidir_today)

mixed_returns <- (all_pair_combined_results$DE_STRATEGY_RETURN+all_pair_combined_results$GB_STRATEGY_RETURN+all_pair_combined_results$JP_STRATEGY_RETURN)/3
equally_mixed_spreadbidir_today <- computeIR(mixed_returns, 21)
print(equally_mixed_spreadbidir_today)


equal_strategy <- CumFromRetToPricesStart(mixed_returns)
plot(all_pair_combined_results$DATES,equal_strategy)
