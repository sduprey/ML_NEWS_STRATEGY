...
# outputDataPathStrategyMonth <- paste(outputDataPath,"Month_2007/01_11_2015_Old/",sep="")
...
late_results <- all_results_df[complete.cases(all_results_df),]


#### Doing the same but with the back files

all_back_results_df <- NULL
...
#   }
  for (nb in 1:nb_deep_solution){
    ...
    if (nb == nb_deep_solution){
      ...
      if (is.null(all_back_results_df)){
        all_back_results_df <- results
      } else {
        all_back_results_df <- merge(all_back_results_df,results,by="DATES",all=T)
      }
...
old_columns <- colnames(all_back_results_df)
back_results <- all_back_results_df[complete.cases(all_back_results_df),]


### Merging our two datasets
results <- rbind(back_results, late_results)

### And recomputing the results from the strategy
oldDE <- results$DE_STRATEGY_TODAY
results$DE_STRATEGY_TODAY <- CumFromRetToPricesStart(results$DE_STRATEGY_RETURN)
results$GB_STRATEGY_TODAY <- CumFromRetToPricesStart(results$GB_STRATEGY_RETURN)
results$JP_STRATEGY_TODAY <- CumFromRetToPricesStart(results$JP_STRATEGY_RETURN)
...
IR_spreadbidir_today <- computeIR(results$STRATEGY_RETURN, biggest_today_IR$horizon)
print("all backtest IR today")
print(IR_spreadbidir_today)
IR_spreadbidir_today_bis <- computeIRfromPrice(results$STRATEGY_TODAY, biggest_today_IR$horizon)
print((IR_spreadbidir_today_bis - IR_spreadbidir_today))
IR_spreadbidir_tomorrow <- computeIRfromPrice(results$STRATEGY_TOMORROW, biggest_today_IR$horizon)
print("all backtest IR tomorrow")
print(IR_spreadbidir_tomorrow)
IR_spreadbidir_yesterday <- computeIRfromPrice(results$STRATEGY_YESTERDAY, biggest_today_IR$horizon)
