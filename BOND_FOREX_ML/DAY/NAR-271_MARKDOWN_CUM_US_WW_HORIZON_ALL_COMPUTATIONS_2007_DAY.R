...
outputDataPathDay <- paste(outputDataPath,"Day_2007/",sep="")
...
options(warn=-1)
my_pair_IR_results_df_name <- "all_pairs_day_2007"
        ...
        for (zscore in to_zscore){
          for (my_horizon in investment_horizon) {
            for (spread_amplification_factor in spread_amplification_factors){
              ...
              #               print(spread_amplification_factor)
              #               
              my_result_spread_name <- paste(my_pair[1], my_pair[2], algorithm,"EU",europe_as_third_country,"ZS",zscore,"depth",depth,"amp",spread_amplification_factor,"inv_hor",my_horizon, "spread_results_day_2007",sep="")
              my_result_spread_filename <- paste(my_result_spread_name, ".rds",sep="")
              
              results <- readRDS(paste(outputDataPathDay,my_result_spread_filename, sep = ""))
...
SaveDataFrame(my_pair_IR_results_df,outputDataPathDay,my_pair_IR_results_df_name)

#### results

all_pair_results <- readRDS(paste(outputDataPathDay,"all_pairs_day_2007.rds", sep = ""))
